...
一个美观、现代的Astro博客主题，支持在线编辑管理内容。

## 功能特点

- 🚀 基于Astro构建，快速高效
- 📝 支持Markdown/MDX格式的博客文章
- 💼 内置管理界面，可在线编辑内容
- 🔄 Supabase后端支持，存储所有内容
- 📱 响应式设计，适配各种设备
- 💬 支持动态内容和朋友圈功能
...
| 列名       | 类型      | 说明                   |
|-----------|-----------|------------------------|
| id        | uuid      | 主键，默认为`uuid_generate_v4()` |
| title     | text      | 文章标题               |
| content   | text      | 文章内容 (Markdown格式) |
| categories| text      | 文章分类               |
| tags      | text[]    | 文章标签 (数组)         |
| cover     | text      | 封面图片URL            |
| date      | timestamp | 发布日期               |
| created_at| timestamp | 创建时间，默认为`now()` |
...
1. 在Supabase项目中启用Email授权
2. 使用"用户管理"功能创建一个管理员用户

## 使用管理界面

完成上述设置后，访问 `/admin` 路径进入管理界面，可以：
...
您可以通过修改`src/config.ts`文件自定义网站的基本信息：
...
export default {
  ...
  Title: '您的博客名称',
  // 网站地址
  Site: 'https://您的网站地址.com',
  // 网站副标题
  Subtitle: '您的博客副标题或格言',
  // 网站描述
  Description: '这里是您的网站描述',
  // 网站作者
  Author: '您的名字',
  // ... 更多配置选项
}
